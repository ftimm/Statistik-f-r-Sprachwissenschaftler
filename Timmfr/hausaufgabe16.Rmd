...
Zusammenhang von Preis und Carat/Klarheit:

```{r}
ggplot(diamonds, aes(x=carat, y=price, color=clarity))+ geom_point(alpha=.5) 
```
Es ist zu sehen, dass die Beziehung zwischen Preis und Volumen der Steine fast exponential verläuft. Zudem sind unter den größten und teuersten Steinen meist Klarheitsgrade von I1, SI1/2 vorhanden.
Um die Ergebnisse schriftlich darzustellen, wird lineare Regression genutzt:

```{r}
summary(lm(price~carat,data=diamonds))
```
Da der Wert von R^2 bei .85 liegt, scheint das Volumen einen großen Einfluss auf den Preis zu haben. 

Die kategoriale Variable "Clarity" wird durch eine Funktion umgewandelt, damit sie analysierbar wird:
```{r}
diamonds$clarity.numeric <- as.numeric(diamonds$clarity)
summary(lm(price~clarity.numeric,data=diamonds))
```
Der niedriege R^2 Wert weist darauf hin, dass die Klarheit der Diamenten wenig Einfluss auf den Preis hat.
