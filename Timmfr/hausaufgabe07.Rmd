...
for(s in c(3,4,5) ){
  ...
  output <- paste("Bei einer Standabweichung von",s, "bestehen",bestanden*100,"% mit sehr guten Noten.")
  print(output)
}
```
...
Es würden weniger Studenten bestehen als durchfallen. Allerdings gleichen sich die Werte an, je höher die Standardabweichung ist. 

Warum?

In die Anzahl von schlechten Noten zählen doppelt so viele (0-5) als in die Anzahl der guten Noten (13-15).
...
Eine Verteilung mit mehr guten Noten ist linksschief.
...
library(ggplot2)
ggplot(data=noten.dist,aes(x=Notenpunkte, y=Anzahl, color=Standardabweichung)) + geom_line() 
...
```{r}
z.test2 <- sqrt(50) * ((10-8)/3)
print(z.test2)
```

Das ist ein **signifikanter** Unterschied. 

Ist die Benotung vom Dozenten weniger als ideal? Ja, da ein zu großer Unterschied zwischen dem Kritischen Wert und dem Z-Wert der Studenten vorhanden ist
...
```{r}
z.test3 <- sqrt(20) * ((7-8)/3)
print(z.test3)
```

Das ist ein **insignifikanter_** Unterschied. 
